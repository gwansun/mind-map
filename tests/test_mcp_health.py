...
def _mock_ollama_up(monkeypatch=None):
    ...
    return {
        "mind_map.processor.processing_llm.check_ollama_available": True,
        "mind_map.processor.processing_llm.get_selected_model": "phi3.5",
        "mind_map.processor.processing_llm.get_available_models": ["phi3.5", "mistral"],
    }
...
def _mock_ollama_down():
    ...
    return {
        "mind_map.processor.processing_llm.check_ollama_available": False,
        "mind_map.processor.processing_llm.get_selected_model": "phi3.5",
        "mind_map.processor.processing_llm.get_available_models": [],
    }
    ...
    def test_custom_workspace(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("mind_map.mcp.server.DEFAULT_DATA_DIR", Path(tmpdir)), \
                 patch("mind_map.processor.processing_llm.check_ollama_available", return_value=False), \
                 patch("mind_map.rag.llm_status.get_llm_status", return_value=_mock_llm_status_offline()):
...
class TestHealthOllamaCheck:
    ...
    def test_ollama_pass_when_running_with_model(self, temp_store: GraphStore):
        with patch("mind_map.processor.processing_llm.check_ollama_available", return_value=True), \
             patch("mind_map.processor.processing_llm.get_selected_model", return_value="phi3.5"), \
             patch("mind_map.processor.processing_llm.get_available_models", return_value=["phi3.5", "mistral"]), \
             patch("mind_map.rag.llm_status.get_llm_status", return_value=_mock_llm_status_offline()):
            result = json.loads(_health())
    ...
    def test_ollama_fail_when_model_missing(self, temp_store: GraphStore):
        with patch("mind_map.processor.processing_llm.check_ollama_available", return_value=True), \
             patch("mind_map.processor.processing_llm.get_selected_model", return_value="nonexistent"), \
             patch("mind_map.processor.processing_llm.get_available_models", return_value=["phi3.5"]), \
             patch("mind_map.rag.llm_status.get_llm_status", return_value=_mock_llm_status_offline()):
            result = json.loads(_health())
            check = result["checks"]["ollama_connection"]
            assert check["status"] == "fail"
    ...
    def test_ollama_exception_handled(self, temp_store: GraphStore):
        with patch("mind_map.processor.processing_llm.check_ollama_available", side_effect=RuntimeError("boom")), \
             patch("mind_map.rag.llm_status.get_llm_status", return_value=_mock_llm_status_offline()):
            result = json.loads(_health())
            check = result["checks"]["ollama_connection"]
            assert check["status"] == "fail"
...
class TestHealthLLMStatus:
    ...
    def test_processing_llm_available(self, temp_store: GraphStore):
        with patch("mind_map.processor.processing_llm.check_ollama_available", return_value=False), \
             patch("mind_map.rag.llm_status.get_llm_status", return_value=_mock_llm_status_online()):
            result = json.loads(_health())
    ...
    def test_llm_status_exception_handled(self, temp_store: GraphStore):
        with patch("mind_map.processor.processing_llm.check_ollama_available", return_value=False), \
             patch("mind_map.rag.llm_status.get_llm_status", side_effect=RuntimeError("status error")):
            result = json.loads(_health())
            assert result["checks"]["processing_llm"]["status"] == "unavailable"
...
class TestHealthErrorHandling:
    ...
    def test_critical_failure_returns_error_json(self):
        ...
        with patch("mind_map.processor.processing_llm.check_ollama_available", side_effect=RuntimeError("total crash")), \
             patch("mind_map.rag.llm_status.get_llm_status", side_effect=RuntimeError("also crashed")), \
