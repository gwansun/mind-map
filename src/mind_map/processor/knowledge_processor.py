...
1. Primary: OpenClaw MiniMax agent subprocess for structured extraction with entity/tag reference context
...
class KnowledgeProcessor:
    ...
    def extract_with_references(
        ...
        minimax_result = self._try_openclaw_minimax_primary(text, reference_nodes)
        if minimax_result is not None:
            return minimax_result

        if self._has_fallback:
            return self._extract_fallback_with_references(text, reference_nodes)
